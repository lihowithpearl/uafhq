  ...
  const processData = () => {
    ...
    Object.keys(departments).forEach((department) => {
      ...
      if (data) {
        ...
        const amStrength = data.match(/AM:\s*(.+)/i)[1] || 'N/A';
        const pmStrength = data.match(/PM:\s*(.+)/i)[1] || 'N/A';
        ...
        {     
          ...
          //   output += `C3+C4 NSF: ${c3c4Nsf-c3c4Reg+1}/${c3c4NsfT-c3c4RegT-1}<br><br>C3+C4 Regular: ${c3c4Reg+1}/${c3c4RegT}<br><br>`;
              ...
              // let regex = `/` + StrgT[i] + `reg strength\s*([\d\d/]+)/i`;         
              // console.log(regex);
              // console.log(data.match(/C1\s*\+\s*C2\s*Reg/i));
              let lines = data.match(/C2 Reg\s*/i)
              ? data.split('C2 Reg')[1].split('\n').slice(1):[];
              for(let line of lines) {
                const [name,status] = line.split('-').map(str => str.trim());
                if(name.match(/NSF/i) != null)
                {
                  reg = false;
                }
                else if(name.match(/Reg/i) != null)
                {
                  reg = true;
                  console.log("reg" + reg);
                ...
                if(name.length > 0 )
                {
                 ...
                 if (reg == true && name.length > 0 && status && !['Present', 'Incoming', 'present'].includes(status) && !status.match(/Incoming/i)) {
